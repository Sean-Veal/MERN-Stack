export const googleClientID: string = process.env.GOOGLE_CLIENT_ID;
export const googleClientSecret: string = process.env.GOOGLE_CLIENT_SECRET;
export const facebookAppId: string = process.env.FACEBOOK_APP_ID;
export const facebookAppSecret: string = process.env.FACEBOOK_APP_SECRET;
export const mongoURI: string = process.env.MONGO_URI;
export const cookieKey: string = process.env.COOKIE_KEY;